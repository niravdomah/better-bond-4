...
Gates checked (in parallel by default):
  ...
  Gate 3: Code Quality - TypeScript, ESLint, Build (also parallel internally)
...
async function runGate3CodeQuality(webDir, signal) {
  ...
  // Run tsc, lint, and build concurrently
  const [tscResult, eslintResult, buildResult] = await Promise.all([
