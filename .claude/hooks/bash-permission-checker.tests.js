...
try {
  ...
  fs.writeFileSync(configPath, JSON.stringify({ git: { autoApproveCommit: true, autoApprovePush: true } }));
  ...
  fs.writeFileSync(configPath, JSON.stringify({ git: { autoApproveCommit: true, autoApprovePush: false } }));
...
const configPath2 = path.join(__dirname, '..', 'preferences.json');
const configExisted2 = fs.existsSync(configPath2);
let configBackup2 = null;
if (configExisted2) {
  configBackup2 = fs.readFileSync(configPath2, 'utf8');
}
try {
  fs.writeFileSync(configPath2, JSON.stringify({ git: { autoApproveCommit: true, autoApprovePush: true } }));
  ...
  try { fs.unlinkSync(configPath2); } catch {}
  if (configExisted2 && configBackup2) {
    fs.writeFileSync(configPath2, configBackup2);
  }
}
